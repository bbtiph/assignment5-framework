...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Transactional
    default void insertWithQuery(User employee) {
    ...
    @Modifying
    @Query(value = "update User u set u.fixedSalary = u.fixedSalary*?1 where u.emplType=?2")
    void rewardSalariedCommissionEmployees(double persent, EmployeeType emplType);

    @Modifying
    @Query(value = "update User u set u.fixedSalary=u.fixedSalary*?1 where u.hoursWorked>=?2 and u.emplType=?3")
    void salaryHourlyEmployees(double coef, int hoursWorked, EmployeeType employeeType);

    @Modifying
    @Query(value = "update User u set u.fixedSalary=u.fixedSalary + (u.fixedSalary*u.commRate) where u.emplType=?1 or u.emplType=?2")
